...
var evaluateFundingOiSignal = (runtime2, body, config) => {
  const thresholds = config.thresholds;
  const weights = config.dataSources.quantFundingOi;
  const decisionMode = readDecisionMode(body.quantDecisionMode);
  const fundingInputBps = readAbsBps(body.mockFundingStressBps ?? body.fundingStressBps ?? body.fundingBps, Math.floor(thresholds.fundingStressBps * 0.7));
  const oiInputBps = readAbsBps(body.mockOiStressBps ?? body.oiStressBps ?? body.oiBps, Math.floor(thresholds.oiStressBps * 0.7));
  const venueDivergenceInputBps = readAbsBps(body.mockVenueDivergenceBps ?? body.venueDivergenceBps ?? body.divergenceBps, Math.floor(thresholds.venueDivergencePenaltyBps * 0.7));
  const weighted = fundingInputBps * weights.fundingWeightBps + oiInputBps * weights.oiWeightBps + venueDivergenceInputBps * weights.divergenceWeightBps;
  const stressScoreBps = Math.floor(weighted / 1e4);
  ...
  const severity = stressScoreBps >= Math.max(thresholds.fundingStressBps, thresholds.oiStressBps) ? "HIGH" : stressScoreBps >= Math.floor(Math.max(thresholds.fundingStressBps, thresholds.oiStressBps) / 2) ? "MEDIUM" : "LOW";
  runtime2.log(`[QUANT_FUNDING_OI_V1][signal] mode=${decisionMode} fundingStressBps=${fundingInputBps} oiStressBps=${oiInputBps} venueDivergenceBps=${venueDivergenceInputBps}`);
  runtime2.log(`[QUANT_FUNDING_OI_V1][signal] weights funding=${weights.fundingWeightBps} oi=${weights.oiWeightBps} divergence=${weights.divergenceWeightBps} stressScoreBps=${stressScoreBps} severity=${severity}`);
  runtime2.log(`[QUANT_FUNDING_OI_V1][signal] earlyWarning base=${thresholds.earlyWarningHfBps} boost=${earlyWarningBoostBps} adjusted=${adjustedEarlyWarningHfBps}`);
  return {
    decisionMode,
    fundingInputBps,
    oiInputBps,
...
var buildFundingOiGuardConfig = (config, signal) => ({
  ...config,
  strategyId: "CHAINLINK_API_GUARD_V1",
  thresholds: {
...
var PROFILE_ID = "QUANT_FUNDING_OI_V1";
var onHttpTrigger = (runtime2, payload) => {
  const body = parseHttpInput(payload);
  runtime2.log(`[${PROFILE_ID}] HTTP trigger received`);
  const quantSignal = evaluateFundingOiSignal(runtime2, body, runtime2.config);
  const guardConfig = buildFundingOiGuardConfig(runtime2.config, quantSignal);
...
var onEvmLogTrigger = (runtime2, payload) => {
  ...
  const quantSignal = evaluateFundingOiSignal(runtime2, {}, runtime2.config);
  const guardConfig = buildFundingOiGuardConfig(runtime2.config, quantSignal);
