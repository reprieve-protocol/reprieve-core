...
import {CCIPClient, ICCIPRouter} from "../../src/reprieve/libs/CCIPClient.sol";
...
contract CrossChainIntegrationTest is Test {
    ...
    // Contracts on "Source Chain" (Arbitrum Sepolia simulation)
    ...
    function setUp() public {
        ...
        BaseLendingEngine engine = destCompoundMarket.engine();
        ...
        // Fund destination executor with debt tokens for repayment
        vm.prank(minter);
        debt.mint(address(destExecutor), 10000e6); // $10k USDC for rescue
    }
    ...
    function test_FullCrossChainRescue() public {
        ...
        steps[0] = ReprieveTypes.RescueStep({
            ...
            collateralAmount: 8 ether,  // Withdraw 8 ETH collateral
            debtAmount: 4000e6,          // Repay 4000 USDC debt
            isCrossChain: true,
        ...
        assertEq(stored.tokenAmounts[0].amount, 8 ether);
        
        // =====================================
        // STEP 2: CCIP Delivers Message
        // =====================================
        
        // Pre-fund the receiver with tokens (simulating CCIP bridge mint)
        vm.prank(minter);
        collateral.mint(address(destReceiver), 8 ether);
        
        // Record balances before delivery
        ...
        // Verify message was delivered (tokens transferred to receiver)
        assertEq(collateral.balanceOf(address(destReceiver)), receiverCollateralBefore + 8 ether);
    ...
    function test_CrossChain_MessageStructure() public {
        ...
        // Data should be decodeable as RescuePayload
        // This would fail if encoding doesn't match
    }
